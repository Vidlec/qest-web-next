...
export class WebStack extends cdk.Stack {
	constructor(
		scope: cdk.Construct,
		id: string,
		{ webBucketLibraryOptions, ...props }: WebStackProps
	) {
		super(scope, id, props)

		const webSiteBucket = new s3.Bucket(
			this,
			`WebSiteBucket-${webBucketLibraryOptions.bucketName}`,
			{
		...
		const hostedZoneForCdn = HostedZone.fromHostedZoneAttributes(
			this,
			'CdnHostedZone',
			{
				hostedZoneId: webBucketLibraryOptions.hostedZoneId,
				zoneName: webBucketLibraryOptions.hostedZone,
			}
		)
		...
		const cdnCertificate = Certificate.fromCertificateArn(
			this,
			'CdnCertificate',
			webBucketLibraryOptions.cdnCertificateArn
		)
		const webBucketLibrary = new WebBucketLibrary(
			this,
			'WebBucketLibrary',
			{
		...
		new CfnOutput(this, 'CdnBaseUrl', {
			value: webBucketLibrary.cdnBaseUrl,
		})
	}
}
