...
const AboutUs: React.FC<{ posts: MediumPost[] }> = ({ posts }) => {
  ...
  const [weAreDescriptionRef, setWeAreDescriptionRef] = useState<HTMLParagraphElement | null>(null)
  ...
  useEffect(() => {
    ...
    return () => {
      for (const descriptionLink of descriptionLinks) {
        descriptionLink.removeEventListener('mouseenter', (e) => moveSlickToIndex(e, sliderRef))
        descriptionLink.addEventListener('mouseleave', () => resumeSlider(sliderRef))
      }
    }
